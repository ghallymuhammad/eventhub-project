...
function EventHomepage() {
  ...
  // Check if categories need scroll button
  useEffect(() => {
  ...
  return (
    <>
                ...
                <div className="relative">
                  ...
                  {showScrollButton && (
                    <button
                      ...
                      className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white/20 hover:bg-white/30 backdrop-blur-sm border border-white/20 hover:border-white/40 text-white p-2 rounded-full transition-all duration-200 shadow-lg hover:shadow-xl z-10"
                  ...
                  {showScrollButton && (
                    <div className="absolute right-0 top-0 w-8 h-full bg-gradient-to-l from-purple-600/30 to-transparent pointer-events-none"></div>
                  )}
                </div>
              </div>
      ...
      <section className="py-8 bg-gradient-to-r from-purple-600 to-pink-600 overflow-hidden">
        <div className="mb-4">
        ...
        >
          {allEvents.filter(event => event.featured).length > 0 ? (
            allEvents
              ...
              .map((event) => (
                <div key={`marquee-${event.id}`} className="mx-6 flex items-center">
                  <Link 
                    href={`/events/${event.id}`}
                    className="flex items-center bg-white/10 backdrop-blur-sm rounded-full px-6 py-3 hover:bg-white/20 transition-all duration-300 border border-white/20 group"
                  >
                    <div className="flex items-center space-x-3">
                      <div className="flex items-center justify-center w-8 h-8 rounded-full bg-yellow-400 text-yellow-900 text-sm font-bold">
                        {getCategoryIcon(event.category)}
                      </div>
                      <div className="text-white">
                        <div className="font-semibold text-sm group-hover:text-yellow-300 transition-colors">
                          {event.title}
                        </div>
            ...
            // Fallback announcements when no featured events are available
            [
              "🎵 New music events added every week!",
              "💻 Tech conferences coming soon - Register now!", 
              "🎨 Art exhibitions open for booking",
              "🍕 Food festivals happening this month",
              "⚽ Sports events with early bird discounts",
              "📚 Educational workshops available"
            ].map((announcement, index) => (
              <div key={`announcement-${index}`} className="mx-6 flex items-center">
