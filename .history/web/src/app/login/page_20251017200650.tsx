  ...
  const handleSubmit = async (values: LoginFormData) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('token', response.data.token); // Fixed token key to match useAuth hook
        localStorage.setItem('user_data', JSON.stringify(response.data.user));
        // Note: Backend doesn't provide refreshToken in login response
        console.log('Login successful, token stored:', response.data.token);
      }
      
      // Redirect to homepage (events)
      router.push('/');
    } catch (error: any) {
