  ...
  const handleSubmit = async (values: LoginFormData) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        ...
        console.log('🔍 Verification - User data stored correctly:', !!storedUser);
      }
      
      // Redirect based on return URL or user role
      const userRole = user.role;
      console.log('🧭 Determining redirect based on role:', userRole);
      ...
      // Force immediate redirect with window.location.href to bypass Next.js routing issues
    ...
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem = function(key: string, value: string) {
        if (key === 'token' || key === 'user_data') {
          console.log('🔄 localStorage.setItem called:', key, value?.substring?.(0, 20) + '...');
        }
      ...
      localStorage.removeItem = function(key: string) {
        if (key === 'token' || key === 'user_data') {
          console.warn('🗑️ localStorage.removeItem called for:', key);
          console.trace('Stack trace for token removal:');
        }
