...
export default function LoginPage() {
  ...
  const handleSubmit = async (values: LoginFormData) => {
    ...
    try {
      ...
      // Use setTimeout to ensure the state has been updated before redirect
      setTimeout(() => {
        console.log('🚀 Executing redirect with timeout...');
        try {
          router.push(redirectUrl);
          console.log('✅ Router.push called successfully');
        } catch (routerError) {
          console.error('⚠️ Router.push failed, using window.location:', routerError);
      ...
      setTimeout(() => {
        if (window.location.pathname === '/login') {
          console.log('🔄 Redirect didn\'t work after 2s, forcing with window.location');
          window.location.href = redirectUrl;
        }
      }, 2000);
