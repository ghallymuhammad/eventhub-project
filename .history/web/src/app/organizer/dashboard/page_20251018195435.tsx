...
export default function OrganizerDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const totalRevenue = mockEvents.reduce((sum, event) => sum + (event.ticketsSold * event.price), 0);
      const upcomingEvents = mockEvents.filter(event => new Date(event.startDate) > new Date()).length;
